-- PROPOSED ONLY. Release H1 (saving fix). Same as 20260925090000 except the total rule is unchanged.
-- The calculation change is held separately in 20260925090500_holiday_grand_total_rule.HELD.sql.
-- Review deployed schema and test on staging before applying.
...
-- SAVING FIX ONLY: keeps today's calculation rules exactly.
-- Today (published app) a holiday save was followed by a browser recalculation:
--   holidays_total := sum(holiday_payments.total)
--   grand_total    := sum(payroll_entries.total_pay) + holidays_total   (timesheet_total NOT written)
-- and the existing payroll_entries trigger (sync_payroll_period_totals, unchanged here) still sets
--   timesheet_total := sum(total_pay), grand_total := timesheet_total.
-- This trigger performs the first rule inside the same transaction as the save. The second rule is untouched.
CREATE OR REPLACE FUNCTION public.sync_holiday_payment_totals_current_rule()
RETURNS trigger LANGUAGE plpgsql SECURITY DEFINER SET search_path = public, pg_temp AS $$
...
BEGIN
  ...
  FOR pid IN SELECT id FROM public.payroll_periods WHERE id IN (old_id, new_id) ORDER BY id FOR UPDATE LOOP
    ...
    UPDATE public.payroll_periods SET holidays_total = holidays, grand_total = worked + holidays WHERE id = pid;
  END LOOP;
  IF TG_OP = 'DELETE' THEN RETURN OLD; END IF;
  RETURN NEW;
END $$;
REVOKE ALL ON FUNCTION public.sync_holiday_payment_totals_current_rule() FROM PUBLIC, anon, authenticated;
...
CREATE TRIGGER trg_sync_holiday_payment_totals AFTER INSERT OR UPDATE OR DELETE
  ON public.holiday_payments FOR EACH ROW EXECUTE FUNCTION public.sync_holiday_payment_totals_current_rule();
