...
import { createHolidayPaymentRunner, HOLIDAY_PAYMENT_RPC } from "@/lib/holiday-payment-transaction";
...
// Each operation is a single database transaction; never fall back to browser writes.
function useHolidayPaymentTransaction() {
...
export function useCreateHolidayPayment() {
  const { run, tenantId, refresh } = useHolidayPaymentTransaction();
  return useMutation({
    mutationFn: async (payment: Omit<HolidayPaymentInsert, "tenant_id">) => {
      await assertPermission("approve_holidays", tenantId!);
      const { id, created_at, ...values } = payment;
      const result = await run(tenantId!, "create", values, id);
...
export function useSettleHolidayPayment() {
  const { run, tenantId, refresh } = useHolidayPaymentTransaction();
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: async (values: Record<string, unknown>) => {
      await assertPermission("approve_holidays", tenantId!);
      return run(tenantId!, "settle", values);
    },
    onSettled: () => {
      refresh();
...
export function useDeleteHolidayPayment() {
  ...
  return useMutation({
    mutationFn: async (paymentId: string) => {
      await assertPermission("approve_holidays", tenantId!);
      const result = await run(tenantId!, "delete", {}, paymentId);
      return { paymentId: result.payment_id, employeeId: result.employee_id };
    },
    onSettled: refresh,
  });
}
...
export function useUpdateHolidayPayment() {
  ...
  return useMutation({
    mutationFn: async ({ id, updates }: { id: string; updates: Partial<{
      hours: number; rate: number; total: number; holiday_taken_date: string;
      leave_year_start: string; leave_year_end: string; notes: string | null;
    }> }) => {
      await assertPermission("approve_holidays", tenantId!);
      const result = await run(tenantId!, "update", updates, id);
      return result.payment!;
    },
    onSettled: refresh,
  });
