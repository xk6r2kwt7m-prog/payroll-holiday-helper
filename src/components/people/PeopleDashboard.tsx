...
export function PeopleDashboard({ onViewDirectory }: { onViewDirectory: (query?: string) => void }) {
  ...
  // Quick actions
  const quickActions = [
    { icon: Calendar, label: "Leave Management", desc: "Approvals, balances and requests", path: "/holidays/manage", color: "text-primary", bg: "bg-primary/8" },
    ...
    { icon: Baby, label: "Absences", desc: "Sickness, lateness and attendance", path: "/absences", color: "text-destructive", bg: "bg-destructive/8" },
  ];
  ...
  if (pendingDocRequests.length > 0) {
    alerts.push({ icon: FileText, label: `${pendingDocRequests.length} missing document${pendingDocRequests.length !== 1 ? "s" : ""}`, color: "text-primary", bg: "bg-primary/10", path: "/employees?view=directory&status=onboarding" });
  }
  ...
  const operationsLinks = [
    ...
    { icon: Scale, label: "Leave Audit", desc: "Balances and history", path: "/holidays/audit" },
  ];
  ...
  return (
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-8 min-w-0">
      <nav aria-label="People views" className="inline-flex rounded-xl bg-muted p-1 gap-1">
        <Button variant="secondary" className="min-h-11" aria-current="page">Overview</Button>
        <Button variant="ghost" className="min-h-11" onClick={() => onViewDirectory()}>Directory</Button>
      </nav>
      {/* Header */}
      <motion.div {...anim} transition={{ duration: 0.25 }}>
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl sm:text-3xl font-semibold text-foreground">People</h1>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            {canEdit && (
              <InviteEmployeeDialog
                trigger={
                  <Button size="icon" variant="outline" aria-label="Invite employee" className="h-11 w-11">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button size="icon" variant="ghost" aria-label="People options" className="h-11 w-11">
      ...
      {alerts.length > 0 && (
        <motion.div {...anim} transition={{ duration: 0.25, delay: 0.06 }}>
          <h2 className="text-base font-semibold text-foreground mb-2">Needs attention</h2>
          <div className="space-y-1.5">
            {alerts.map((alert, i) => (
              <Link
                ...
                to={alert.path}
                className={cn(
                  "flex items-center gap-3 p-3.5 rounded-xl border border-border shadow-sm hover:border-primary/40 active:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring",
                  alert.bg
                )}
              >
      ...
      <form onSubmit={(event) => { event.preventDefault(); onViewDirectory(directorySearch); }} className="flex flex-col sm:flex-row gap-2 rounded-xl border border-border bg-card p-4">
        <label htmlFor="people-search" className="sr-only">Find an employee</label>
        <Input id="people-search" value={directorySearch} onChange={event => setDirectorySearch(event.target.value)} placeholder="Find someone by name, department or reference" className="h-12 flex-1" />
        <Button type="submit" className="min-h-12">Find employee</Button>
      </form>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {quickActions.map((action) => (
            <Link
              key={action.label}
              to={action.path}
              className="rounded-2xl bg-card border border-border p-4 shadow-sm hover:border-primary/40 active:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
      ...
      <motion.div {...anim} transition={{ duration: 0.25, delay: 0.08 }}>
        <h2 className="text-base font-semibold text-foreground mb-2">People operations</h2>
        <div className="space-y-1.5">
          {operationsLinks.map((op) => (
            <Link
              key={op.label}
              to={op.path}
              className="flex items-center gap-3 p-3.5 rounded-xl bg-card border border-border shadow-sm hover:border-primary/40 active:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
      ...
      {previewEmployees.length > 0 && (
        <motion.div {...anim} transition={{ duration: 0.25, delay: 0.1 }}>
          ...
          <div className="space-y-1.5">
            {previewEmployees.map((emp) => (
              <Link
                ...
                className="flex items-center gap-3 p-3 rounded-xl bg-card border border-border shadow-sm hover:border-primary/40 active:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
              >
                ...
                <Badge variant="outline" className="text-[10px] shrink-0 bg-success/10 text-success border-success/20">
                  {emp.status === "active" ? "Active" : emp.status === "starter" ? "Starter" : emp.status === "onboarding" ? "Onboarding" : emp.status}
                </Badge>
              </Link>
            ))}
          </div>
          <button
            onClick={() => onViewDirectory()}
            className="w-full mt-2.5 py-3 rounded-xl bg-card border border-border text-sm font-medium text-primary hover:border-primary/40 active:bg-muted transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring shadow-sm"
          >
            View all employees →
          </button>
        </motion.div>
