...
const CAPABILITIES = [
  {
    ...
    desc: "Plan rotas, track attendance and align labour to demand.",
  ...
  },
  {
    ...
    desc: "Run payroll with operational context managers actually use.",
  ...
  },
  {
    ...
    desc: "Know who is trained, overdue or not yet ready for the rota.",
  ...
  },
  {
    ...
    desc: "Track documents, expiry dates and right-to-work properly.",
  ...
  },
  {
    ...
    desc: "See where standards hold and where issues are building.",
  ...
  },
  {
    ...
    desc: "One view of what is overdue, incomplete or unresolved.",
        ...
        {CAPABILITIES.map((v, i) => {
          ...
          return (
            <button
              ...
              className={`group text-left rounded-xl border p-5 transition-all duration-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${
                isExpanded
            ...
            >
              <div className="flex items-start justify-between gap-2">
                <div className={`flex h-9 w-9 items-center justify-center rounded-lg transition-colors duration-300 ${
                  isExpanded ? "bg-primary/15" : "bg-primary/[0.08] group-hover:bg-primary/15"
                }`}>
                ...
                <ChevronDown className={`h-3.5 w-3.5 text-muted-foreground/40 shrink-0 mt-1 transition-transform duration-300 ${
                  isExpanded ? "rotate-180 text-primary" : ""
                }`} aria-hidden="true" />
              </div>
              <h3 className="text-sm font-bold text-foreground tracking-tight mt-3">{v.title}</h3>
              <p className="text-[12px] sm:text-[13px] text-muted-foreground leading-relaxed mt-1.5">{v.desc}</p>
              <div className={`overflow-hidden transition-all duration-300 ${
                isExpanded ? "max-h-40 opacity-100 mt-3" : "max-h-0 opacity-0"
              }`}>
                <p className="text-[12px] sm:text-[13px] text-primary/80 leading-relaxed border-t border-primary/10 pt-3">
                  {v.detail}
                </p>
              </div>
            </button>
